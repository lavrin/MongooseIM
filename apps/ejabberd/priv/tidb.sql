...
CREATE UNIQUE INDEX i_private_storage_username_namespace ON private_storage(username(75), namespace(75));
...
CREATE TABLE mam_message(
  ...
  INDEX i_mam_message_rem (user_id, remote_bare_jid, id)
)  ENGINE=InnoDB
...
CREATE INDEX i_offline_message ON offline_message(server, username, id);
...
CREATE INDEX i_muc_light_rooms ON muc_light_rooms(id);
...
CREATE TABLE muc_light_occupants(
    ...
    FOREIGN KEY (room_id) REFERENCES muc_light_rooms(id)
);
